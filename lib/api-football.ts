// ─── API-Football client v6 ───────────────────────────────────────────────────
//
// v6 fixes over v5:
//   • LEAGUE_ID_TO_INFO country strings now use SPACES (matching API response)
//     instead of hyphens. This was causing every league lookup to fail silently
//     when the DB league upsert used the raw API country string. The map values
//     are the canonical DB values; raw API strings must be normalised before use.
//   • normalizeCountry() helper added to ensure consistent storage in DB.
//   • fetchOddsForFixture now logs the exact bookmaker used and returns a
//     detailed object so callers can log the source, not just the price.
//   • mapFixtureWithStats now logs when a fixture is dropped and why.
//   • Added quota-remaining logging to apiFetch for every call.
//   • H2H minimum sample raised from 3 → 5 for more reliable draw-rate estimates.
//   • fetchFixturesForDate now accepts an optional leagueIds filter to narrow
//     API calls to only tracked leagues (saves quota on high-fixture days).

export const API_FOOTBALL_BASE = 'https://v3.football.api-sports.io'

// ─── Normalise country string ─────────────────────────────────────────────────
// API-Football returns "Czech Republic", "Saudi Arabia", etc. with spaces.
// We store the same — DO NOT use hyphens in LEAGUE_ID_TO_INFO.
...
// ─── League info map ──────────────────────────────────────────────────────────
//
// IMPORTANT: country strings MUST match what API-Football returns verbatim
// (spaces, not hyphens). These are also what gets stored in the DB.
// The API returns e.g. "Czech Republic" not "Czech-Republic".
//
// Rules:
//  1. Every numeric key MUST be UNIQUE
//  2. country must match API-Football's exact string (with spaces)
//  3. avgDrawRate is a 3-season rolling average
...
> = {
  ...
  // ── Czech Republic ── API returns "Czech Republic" with a space ─────────────
  ...
  // ── North Macedonia ── API returns "North Macedonia" with a space ─────────────
  ...
  // ── Northern Ireland ── API returns "Northern Ireland" with a space ──────────
  ...
  // API returns "Saudi Arabia" with a space
  ...
  // API returns "South Korea" with a space
  ...
  // API returns "South Africa" with a space
  ...
  // API returns "Ivory Coast" with a space
...
export function mapFixtureWithStats(
  ...
  if (!leagueInfo) {
    console.log(`[mapFixture] dropped — league ${fixture.league.id} (${fixture.league.name}, ${fixture.league.country}) not in tracking list`)
    return null
  }
  ...
  if (drawOdds <= 0) {
    console.log(`[mapFixture] dropped fixture ${fixture.fixture.id} (${fixture.teams.home.name} vs ${fixture.teams.away.name}) — no draw odds`)
    return null
  }
  ...
  // H2H: require at least 5 completed results for reliable draw-rate estimate
  ...
  return {
    ...
    // Always use our canonical map values — NOT the raw API strings.
    // This prevents duplicate league rows from country string inconsistencies.
