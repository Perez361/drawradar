// ─── API-Football client v8 ───────────────────────────────────────────────────
//
// Free plan constraints fixed:
//   1. Rate limit 10 req/min → odds fetched SEQUENTIALLY with 6.5s delay.
//   2. Season cap → FREE_PLAN_SEASON = 2024 (free plan: 2022-2024 only).
//   3. H2H `last` param → removed (not on free plan); slice results instead.
//   4. Team stats & form fixtures also use FREE_PLAN_SEASON.
//   5. Rate-limit errors in response body are detected and thrown so the
//      sequential fetcher can stop cleanly instead of wasting quota.

export const API_FOOTBALL_BASE = 'https://v3.football.api-sports.io'

// Free plan only allows seasons 2022–2024
export const FREE_PLAN_SEASON = 2024

export function normalizeCountry(raw: string): string { return raw.trim() }

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms))
}
// 6.5 s between requests → safely under 10 req/min
...
export interface H2HResult {
  ...
  homeGoals: number; awayGoals: number; isDraw: boolean
}
...
export const LEAGUE_ID_TO_INFO: Record<number, { name: string; country: string; avgDrawRate: number }> = {
  ...
  137: { name: 'Coppa Italia',               country: 'Italy',          avgDrawRate: 0.30 },
  942: { name: 'Serie C - Group B',          country: 'Italy',          avgDrawRate: 0.31 },
  943: { name: 'Serie C - Group C',          country: 'Italy',          avgDrawRate: 0.31 },
  426: { name: 'Serie D - Group A',          country: 'Italy',          avgDrawRate: 0.30 },
  427: { name: 'Serie D - Group B',          country: 'Italy',          avgDrawRate: 0.30 },
  428: { name: 'Serie D - Group C',          country: 'Italy',          avgDrawRate: 0.30 },
  429: { name: 'Serie D - Group D',          country: 'Italy',          avgDrawRate: 0.30 },
  430: { name: 'Serie D - Group E',          country: 'Italy',          avgDrawRate: 0.30 },
  431: { name: 'Serie D - Group F',          country: 'Italy',          avgDrawRate: 0.30 },
  432: { name: 'Serie D - Group G',          country: 'Italy',          avgDrawRate: 0.30 },
  433: { name: 'Serie D - Group H',          country: 'Italy',          avgDrawRate: 0.30 },
  434: { name: 'Serie D - Group I',          country: 'Italy',          avgDrawRate: 0.30 },
  ...
  67:  { name: 'National 2 - Group A',       country: 'France',         avgDrawRate: 0.31 },
  68:  { name: 'National 2 - Group B',       country: 'France',         avgDrawRate: 0.31 },
  69:  { name: 'National 2 - Group C',       country: 'France',         avgDrawRate: 0.31 },
  70:  { name: 'National 2 - Group D',       country: 'France',         avgDrawRate: 0.31 },
  ...
  906: { name: 'Reserve League',                country: 'Argentina',      avgDrawRate: 0.30 },
  1178:{ name: 'Supercopa Internacional',          country: 'Argentina',      avgDrawRate: 0.30 },
  ...
  77:  { name: 'Alagano',                    country: 'Brazil',         avgDrawRate: 0.31 },
...
async function apiFetch<T>(apiKey: string, path: string): Promise<T> {
  ...
  console.log(`[API-Football] ${path} — HTTP ${res.status} | used: ${used ?? 'n/a'}, remaining: ${remaining ?? 'n/a'}`)
  ...
  if (json.errors && Object.keys(json.errors).length > 0) {
    ...
    if (errStr.toLowerCase().includes('ratelimit') || errStr.toLowerCase().includes('rate limit')) {
      throw new Error(`RATE_LIMIT: ${errStr}`)
    }
    if (errStr.toLowerCase().includes('plan')) {
      console.warn(`[API-Football] Plan restriction on ${path} — skipping`)
    }
  }
  const count = json.results ?? (Array.isArray(json.response) ? json.response.length : '?')
  console.log(`[API-Football] ${path} — results: ${count}`)
...
export async function fetchFixturesForDate(apiKey: string, date: string): Promise<ApiFootballFixture[]> {
  ...
  const fixtures = await apiFetch<ApiFootballFixture[]>(apiKey, `/fixtures?date=${date}&timezone=UTC`)
  console.log(`[API-Football] fetchFixturesForDate(${date}) → ${fixtures.length} fixtures`)
  if (fixtures.length === 0) console.warn(`[API-Football] 0 fixtures for ${date}. Check quota / match schedule.`)
  return fixtures
}

// ─── Odds — SEQUENTIAL to respect 10 req/min free plan limit ─────────────────
...
function extractDrawOdds(bm: ApiFootballOdds['bookmakers'][0]): number {
  ...
  return draw ? parseFloat(draw.odd) : 0
}

const PRIORITY_BOOKS = ['pinnacle','bet365','williamhill','william hill','bwin','unibet',
  'betfair exchange','betfair','marathonbet','betsson','1xbet','nordicbet','betway','parimatch']

export async function fetchOddsForFixture(apiKey: string, fixtureId: number): Promise<number> {
  const data = await apiFetch<ApiFootballOdds[]>(apiKey, `/odds?fixture=${fixtureId}&bet=1`)
  const bookmakers = data[0]?.bookmakers ?? []
  if (bookmakers.length === 0) { console.log(`[odds] fixture ${fixtureId} — no bookmakers`); return 0 }
  console.log(`[odds] fixture ${fixtureId} — ${bookmakers.length} books: ${bookmakers.map(b => b.name).join(', ')}`)
  for (const preferred of PRIORITY_BOOKS) {
    for (const bm of bookmakers) {
      if (bm.name.toLowerCase().includes(preferred)) {
        const odds = extractDrawOdds(bm)
        if (odds > 0) { console.log(`[odds] fixture ${fixtureId} — ${bm.name}: ${odds}`); return odds }
      }
    }
  }
  const sorted = bookmakers.map(bm => extractDrawOdds(bm)).filter(o => o >= 1.2 && o <= 20).sort((a,b)=>a-b)
  if (sorted.length === 0) return 0
  const trim = Math.floor(sorted.length * 0.15)
  const src = sorted.slice(trim, sorted.length - trim)
  const use = src.length > 0 ? src : sorted
  const mid = Math.floor(use.length / 2)
  const median = use.length % 2 !== 0 ? use[mid] : (use[mid-1] + use[mid]) / 2
  const result = Math.round(median * 100) / 100
  console.log(`[odds] fixture ${fixtureId} — median from ${use.length} books: ${result}`)
  return result
}

/**
 * Fetch odds for multiple fixtures SEQUENTIALLY with rate-limit delay.
 * Free plan: 10 req/min. We wait 6.5s between each call.
 * Stops early if a rate-limit error is thrown; returns partial results.
 */
export async function fetchOddsForFixtures(
  ...
  for (let i = 0; i < fixtureIds.length; i++) {
    if (i > 0) {
      console.log(`[odds] rate-limit delay before fixture ${fixtureIds[i]} (${i+1}/${fixtureIds.length})…`)
      await sleep(RATE_LIMIT_DELAY_MS)
    }
    try {
      const odds = await fetchOddsForFixture(apiKey, fixtureIds[i])
      if (odds > 0) result.set(fixtureIds[i], odds)
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      if (msg.startsWith('RATE_LIMIT')) {
        console.error(`[odds] Rate limit hit at index ${i}. Stopping with ${result.size} results.`)
        break
      }
      console.warn(`[odds] fixture ${fixtureIds[i]} failed:`, msg)
    }
  }
  console.log(`[odds] Sequential fetch done: ${result.size}/${fixtureIds.length} fixtures have odds`)
  return result
}

// ─── H2H — no `last` param (not on free plan) ────────────────────────────────

export async function fetchH2H(
  apiKey: string,
  homeTeamId: number,
  awayTeamId: number,
  _last = 10   // ignored — free plan doesn't support `last`
): Promise<H2HResult[]> {
  try {
    await sleep(RATE_LIMIT_DELAY_MS)
    // Use FREE_PLAN_SEASON; omit `last` param
    const raw = await apiFetch<ApiFootballFixture[]>(
      apiKey,
      `/fixtures/headtohead?h2h=${homeTeamId}-${awayTeamId}&season=${FREE_PLAN_SEASON}`
    )
    return raw
...
// ─── Recent fixtures — uses FREE_PLAN_SEASON, no `last` param ────────────────

export interface RecentFixture {
  date: string; goalsScored: number; goalsConceded: number; isDraw: boolean
}
...
export async function fetchRecentFixtures(
  ...
  teamId: number,
  leagueId: number,
  _season: number,  // ignored on free plan
  last = 10
): Promise<RecentFixture[]> {
  try {
    await sleep(RATE_LIMIT_DELAY_MS)
    // Use FREE_PLAN_SEASON; omit `last` and `status` params that may not be free
    const raw = await apiFetch<ApiFootballFixture[]>(
      apiKey,
      `/fixtures?team=${teamId}&league=${leagueId}&season=${FREE_PLAN_SEASON}`
    )
    return raw
      .filter(f => f.goals.home !== null && f.goals.away !== null &&
...
export interface MappedFixture {
  external_id: string; fixture_id: number
  home_team_id: number; away_team_id: number
  league_id_ext: number; league_name: string; league_country: string
  home_team_name: string; away_team_name: string
  match_date: string; draw_odds: number
  home_goals_avg: number; away_goals_avg: number
  home_concede_avg: number; away_concede_avg: number
  home_draw_rate: number; away_draw_rate: number
  h2h_draw_rate: number; h2h_is_real: boolean
  xg_home: number; xg_away: number; avg_draw_rate: number
  has_real_team_stats: boolean
  home_form_draw_rate: number | null; away_form_draw_rate: number | null
  home_form_goals_avg: number | null; away_form_goals_avg: number | null
  home_games_last14: number | null; away_games_last14: number | null
  odds_open: number | null; odds_movement: number | null
}
export interface PerTeamStats { goalsForAvg: number; goalsAgainstAvg: number; drawRate: number }
export interface FormStats    { formDrawRate: number; formGoalsAvg: number; gamesLast14: number }
...
export function mapFixtureWithStats(
  ...
  if (!leagueInfo) { console.log(`[mapFixture] dropped — league ${fixture.league.id} not tracked`); return null }
  const status = fixture.fixture.status.short
  if (!['NS','TBD'].includes(status)) { console.log(`[mapFixture] dropped ${fixture.fixture.id} — status '${status}'`); return null }
  if (drawOdds <= 0) { console.log(`[mapFixture] dropped ${fixture.fixture.id} — no odds`); return null }
  ...
  if (h2hResults && h2hResults.length >= 5) {
    const sorted = [...h2hResults].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime())
    let wDraws = 0, wTotal = 0
    sorted.slice(0, 10).forEach((r, i) => { const w = i < 5 ? 2 : 1; wDraws += w*(r.isDraw?1:0); wTotal += w })
    h2hDrawRate = Math.round((wDraws / wTotal) * 1000) / 1000
    h2hIsReal = true
  }

  const oddsMovement = openingDrawOdds && openingDrawOdds > 0
    ? Math.round((drawOdds - openingDrawOdds) * 100) / 100 : null

  return {
    external_id: `apifb_${fixture.fixture.id}`, fixture_id: fixture.fixture.id,
    home_team_id: fixture.teams.home.id, away_team_id: fixture.teams.away.id,
    league_id_ext: fixture.league.id, league_name: leagueInfo.name, league_country: leagueInfo.country,
    home_team_name: fixture.teams.home.name, away_team_name: fixture.teams.away.name,
    match_date: fixture.fixture.date, draw_odds: drawOdds,
    home_goals_avg: homeGoalsFor, away_goals_avg: awayGoalsFor,
    home_concede_avg: homeConcede, away_concede_avg: awayConcede,
    home_draw_rate: homeDrawRate, away_draw_rate: awayDrawRate,
    h2h_draw_rate: h2hDrawRate, h2h_is_real: h2hIsReal,
    xg_home: xgHome, xg_away: xgAway, avg_draw_rate: avg,
    has_real_team_stats: homeStats !== null || awayStats !== null,
    home_form_draw_rate: homeForm?.formDrawRate ?? null, away_form_draw_rate: awayForm?.formDrawRate ?? null,
    home_form_goals_avg: homeForm?.formGoalsAvg ?? null, away_form_goals_avg: awayForm?.formGoalsAvg ?? null,
    home_games_last14: homeForm?.gamesLast14 ?? null, away_games_last14: awayForm?.gamesLast14 ?? null,
    odds_open: openingDrawOdds ?? null, odds_movement: oddsMovement,
  }
}
...
function getLeagueGoalsAvg(leagueId: number, side: 'home' | 'away'): number {
  const b: Record<number, [number, number]> = {
    ...
    135:[1.49,1.19],136:[1.38,1.08],61:[1.51,1.18],62:[1.40,1.10],63:[1.38,1.08],
    88:[1.81,1.44],89:[1.65,1.30],94:[1.48,1.14],95:[1.42,1.12],96:[1.38,1.10],
    203:[1.56,1.24],204:[1.48,1.18],205:[1.42,1.12],144:[1.62,1.28],145:[1.50,1.20],
    179:[1.55,1.20],180:[1.48,1.18],197:[1.43,1.10],198:[1.38,1.08],
    235:[1.50,1.18],236:[1.42,1.12],113:[1.60,1.25],114:[1.52,1.20],
    119:[1.58,1.22],120:[1.50,1.18],103:[1.55,1.20],104:[1.50,1.18],
    106:[1.46,1.14],107:[1.40,1.10],345:[1.50,1.16],346:[1.45,1.14],
    283:[1.42,1.12],284:[1.38,1.08],172:[1.40,1.10],218:[1.46,1.12],
    210:[1.48,1.14],211:[1.42,1.12],128:[1.50,1.18],129:[1.35,1.05],
    131:[1.28,1.00],132:[1.28,1.00],71:[1.55,1.20],72:[1.40,1.10],
    75:[1.35,1.05],76:[1.28,1.00],265:[1.45,1.12],266:[1.40,1.10],
    239:[1.48,1.15],240:[1.45,1.12],242:[1.40,1.10],268:[1.42,1.12],
    269:[1.38,1.08],299:[1.36,1.06],344:[1.30,1.02],252:[1.38,1.08],
    281:[1.40,1.10],262:[1.48,1.15],253:[1.40,1.10],307:[1.55,1.22],
    308:[1.45,1.14],305:[1.38,1.08],290:[1.35,1.05],98:[1.50,1.18],
    99:[1.45,1.14],100:[1.40,1.12],292:[1.48,1.15],293:[1.40,1.10],
    169:[1.45,1.15],170:[1.38,1.08],323:[1.42,1.12],324:[1.38,1.10],
    274:[1.38,1.08],297:[1.38,1.08],340:[1.36,1.06],188:[1.40,1.10],
    233:[1.30,1.02],200:[1.33,1.06],186:[1.28,1.00],288:[1.38,1.10],
    289:[1.34,1.08],399:[1.30,1.02],
  }
  const pair = b[leagueId] ?? [1.45,1.18]
